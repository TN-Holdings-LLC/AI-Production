...
Traditional Transformers use `softmax` to calculate attention weights. However, `softmax` never truly reduces any probability to zero. It preserves invalid, noisy data as microscopic probabilities (e.g., $0.0001\%$), which are then aggregated into the final output. This accumulation of "noise" is the root cause of AI hallucinations.
